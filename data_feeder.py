...
class TrainSlidingWindowGenerator():
    ...
    def __init__(self, 
                ...
                crop=100000, 
                skip_rows=0, 
                ram_threshold = 5 * 10 ** 6):
        self.__file_name = file_name
        self.__batch_size = batch_size
        self.__chunk_size = chunk_size      #10 ** 6
    ...
    def check_if_chunking(self):
        ...
        chunks = pd.read_csv(self.__file_name, header=None, nrows=self.__crop, skiprows=self.__skip_rows)
    ...
    def load_dataset(self):
        ...
        if (self.total_size <= self.__ram_threshold):
            ...
            data_array = np.array(pd.read_csv(self.__file_name, nrows=self.__crop, skiprows=self.__skip_rows, header=None))
            inputs = data_array[:, 0]
...
class TestSlidingWindowGenerator(object):
    ...
    def load_dataset(self):
        ...
        if self.__number_of_windows < 0: #means batch_sizes in train_tester
        ...
        for start_index in range(0, max_number_of_windows, self.__number_of_windows):
            ...
            #(number_of_windows,window),(number_of_windows,1)
