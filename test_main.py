...
# appliances = ['microwave','dishwasher','washingmachine']
# appliances = ['fridge']
...
for applianc_name in appliances:
    ...
    saved_model_dir = experiment_dir + "{}/".format(dataset) + arguments.appliance_name + "_" + arguments.algorithm + "_model.h5"

    # The logs including results will be recorded to this log file
    log_file_dir = experiment_dir + "{}/test/".format(dataset) + arguments.algorithm + ".log"
    # log_file_dir = "experiments/exp210215/{}_{}.log".format(arguments.algorithm, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))

    fig_path = experiment_dir +  "{}/test/".format(dataset)
